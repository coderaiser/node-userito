...
export const createFile = (filePath, schema) => {
    if (filePath)
...
File.prototype.all = async function() {
    return await readjson(this._path);
};

File.prototype.get = async function(username) {
...
File.prototype.create = async function(params) {
...
File.prototype.update = async function(username, data) {
    const schema = this._schema;
    const path = this._path;
...
File.prototype.remove = async function(username) {
    const path = this._path;
...
export function _createFile(filePath, overrides = {}) {
    const {read = readjson.sync, write= writejson.sync} = overrides;
