...
export const createUserito = (options, overrides) => {
    if (!options)
        throw Error('options could not be empty!');
    
    return new Userito(options, overrides);
};

function Userito(options, overrides = {}) {
    const {type} = options;
    const {users} = overrides;
    ...
    if (type === 'mongo') {
        if (!options.mongo)
            throw Error('options.mongo could not be empty!');
        
        this._users = createMongo(options.mongo, options.schema);
...
Userito.prototype.get = async function(username) {
    const get = this._users.get.bind(this._users);
